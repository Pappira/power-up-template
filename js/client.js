...
var pdf = function(){
  return  {
    ...
    callback: function(t){
      ...
      doc.fromHTML('<footer><div style="text-align:center;">Page <span class="pageCounter"></span>/<span class="totalPages"></span></div></footer>', 1, 1, {'width':29, 'elementHandlers': {}});
      doc.save('demo.pdf');
    }
  };
...
var getAddPrices = function(){
  return function(t){
    return t.modal({
      url: './price.html', // The URL to load for the iframe
      accentColor: '#F2D600', // Optional color for the modal header 
      height: 500, // Initial height for iframe; not used if fullscreen is true
      fullscreen: false, // Whether the modal should stretch to take up the whole screen
      callback: () => console.log('Goodbye.'), // optional function called if user closes modal (via `X` or escape)
      title: 'Agregar Precios', // Optional title for modal header
      // You can add up to 3 action buttons on the modal header - max 1 on the right side.
      actions: [{
        icon: GRAY_ICON,
        url: 'https://google.com', // Opens the URL passed to it.
