...
var createOptionalFinishesText = function(estimate,dontTakeCareOfSelectedOption){
	...
	if (estimate.optionalFinishes && estimate.optionalFinishes.length >0){
		...
		texts = [].concat.apply([],finishes.map(finish => [].concat.apply([],[createText('subtitle3','Opcional ' + (finish.item!=-1?estimate.items[finish.item].name+' ':'') + finish.finish,''), finish.price.map(price => createText('text','Sub-Total extra (' + price.quantity + ' unidades)', '$ ' +  price.price.toLocaleString() + ' + IVA'))])))
	}
	return texts;
}
...
var getCombinationsForWork = function(estimate){
  var items= [];
  ...
  for (var i = 0; i  < estimate.quantities.length; i++){
    quantities.push('{"quantity":' + estimate.quantities[i]+',');
  }
